...
REMOTE_HOST_MACHINE_SSH_CONNECTION = "ssh_recsys_prod_connection_backup" #Variable.get("REMOTE_HOST_MACHINE_SSH_CONNECTION")
...
with DAG(
    ...
    repository_name, clone_project = create_clone_repository_operator(
        ...
        dag_id="{{ dag.dag_id }}",
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
        branch_name="feature/historytape_test",
    )
    ...
    #
    # build_historytape = create_python_in_docker_ssh_command(
    #     task_id="build_historytape",
    #     command="python -m vvrecsys.cli.production --config=configs/production/historytape_test.yaml",
    #     docker_container_name=docker_container_name,
    #     cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
    #     ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    # )

    deploy_historytape = create_ssh_command_operator(
        task_id="deploy_historytape",
        command=(
            f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config=configs/production/historytape_test.yaml --index=0 --focus_group=0"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
    ...
    # recommends_folder = "/srv/data/production/historytape"
    # s3_folder = "recsys3/models/historytape"
    #
    ...
    #     aws s3 cp /srv/data/production/historytape/ s3://{{ params.bucket_name }}/recsys3/production/{{ macros.ds_add(ds, -1).replace('-', '/') }}/historytape/ --recursive --exclude '*' --include '*.parquet' --endpoint-url=https://storage.yandexcloud.net
    #     """,
    #     params={
