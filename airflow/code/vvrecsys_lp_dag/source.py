...
with DAG(
    ...
    build_lp = create_python_in_docker_ssh_command(
        task_id="build_lp",
        command="python -m vvrecsys.cli.production --config=configs/production/lp.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    deploy_lp = create_ssh_command_operator(
        task_id="deploy_lp",
        command=(
            f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config=configs/production/lp.yaml"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
    ...
    recommends_folder = "/srv/data/production/lp"
    s3_folder = "recsys3/models/lp"
    ...
    copy_files_to_s3 = SSHOperator(
        ...
        aws s3 cp /srv/data/production/lp/ s3://{{ params.bucket_name }}/recsys3/production/{{ macros.ds_add(ds, -1).replace('-', '/') }}/lp/ --recursive --exclude '*' --include '*.h5' --endpoint-url=https://storage.yandexcloud.net
        """,
        params={
