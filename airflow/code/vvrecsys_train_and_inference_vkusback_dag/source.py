...
model_configs = [
    {
        "model_name": "vkusback",
        "train_task": {
            "task_id": "train_vkusback",
            "command": "python -m vvrecsys.cli.training --config=configs/models/vkusback.yaml",
            "timeout": 1 * 60 * 60 * 10,  # 10 minutes
        },
        "predict_task": {
            "task_id": "predict_vkusback",
            "command": "python -m vvrecsys.cli.inference --config=configs/models/vkusback.yaml --K=1000 --date {{ params.l_date if params.l_date else macros.ds_add(data_interval_end | ds, -1) }} --force",
...
with DAG(
    "vvrecsys_train_and_inference_vkusback_dag",
