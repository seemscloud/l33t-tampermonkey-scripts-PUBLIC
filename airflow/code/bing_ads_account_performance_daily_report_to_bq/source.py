"""
    https://learn.microsoft.com/en-us/advertising/reporting-service/accountperformancereportcolumn?view=bingads-13#lowqualityconversionrate
"""
import json
import requests
import os
...
last_30_day_ago = (datetime.today() - timedelta(days=31)).strftime('%Y-%m-%d')
yesterday = (datetime.today() - timedelta(days=1)).strftime('%Y-%m-%d')
today_date = datetime.today().strftime('%Y-%m-%d')
bq_project = GCPConnectionInfo.gcp_project_test
bq_dataset = 'ads_msft'
bq_table = 'account_performance_daily_report'
    ...
    if raw_data:
        for row in raw_data:
            df_row_data = {
                ...
                "ad_distribution": row.get('AdDistribution'),
                "bid_match_type": row.get('BidMatchType'),
                "currency_code": row.get('CurrencyCode'),
                "date": row.get('TimePeriod'),
                "delivered_match_type": row.get('DeliveredMatchType'),
                "device_os": row.get('DeviceOS'),
                "device_type": row.get('DeviceType'),
                "network": row.get('Network'),
                "top_vs_other": row.get('TopVsOther'),
                ...
                "average_position": row.get('AveragePosition'),
                ...
                "low_quality_clicks": row.get('LowQualityClicks'),
                "low_quality_clicks_percent": row.get('LowQualityClicksPercent'),
                "low_quality_conversion_rate": row.get('LowQualityConversionRate'),
                "low_quality_conversions": row.get('LowQualityConversions'),
                "low_quality_conversions_qualified": row.get('LowQualityConversionsQualified'),
                "low_quality_general_clicks": row.get('LowQualityGeneralClicks'),
                "low_quality_impressions": row.get('LowQualityImpressions'),
                "low_quality_impressions_percent": row.get('LowQualityImpressionsPercent'),
                "low_quality_sophisticated_clicks": row.get('LowQualitySophisticatedClicks'),
                "phone_calls": row.get('PhoneCalls'),
                "phone_impressions": row.get('PhoneImpressions'),
...
def remove_old_data():
    GCPConnectionInfo().execute_query(
        f'DELETE FROM {bq_project}.{bq_dataset}.{bq_table} WHERE date >= "{last_30_day_ago}"')
    return True
...
dag = DAG(
    dag_id='bing_ads_account_performance_daily_report_to_bq',
    schedule='15 0 * * *',
