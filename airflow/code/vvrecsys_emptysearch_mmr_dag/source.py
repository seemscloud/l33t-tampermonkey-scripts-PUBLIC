...
with DAG(
    ...
    build_emptysearch_new_filtering = create_python_in_docker_ssh_command(
    ...
    build_emptysearch_mrr = create_python_in_docker_ssh_command(
        task_id="build_emptysearch_mrr",
        command="python -m vvrecsys.cli.production --config=configs/production/emptysearch_mmr.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 5,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
    ...
    (
        ...
        >> start_docker
        >> set_date
        >> build_emptysearch_mrr
        >> build_emptysearch_new_filtering
        >> stop_docker
       # >> copy_files_to_s3
    )
