...
with DAG(
    "vvrecsys_analogs_text_dag",
    default_args={
        "depends_on_past": False,
        "on_failure_callback": create_telegram_alert_func(TELEGRAM_TOKEN_BOT),
        "owner": "v.zelenin",
    },
    description="DAG for inference analogs_text recs",
    schedule_interval="15 0 * * *",
    start_date=datetime(2021, 1, 1),
    catchup=False,
    tags=["recsys", "analogs_text", "prod"],
) as dag:
    start = DummyOperator(task_id="start")

    year = "{{ ds.split('-')[0] }}"
    month = "{{ ds.split('-')[1].zfill(2) }}"
    day = "{{ ds.split('-')[2].zfill(2) }}"

    s3_sensor = S3KeySensor(
        task_id="check_s3_files",
        bucket_name=BUCKET_NAME,
        bucket_key=f"recsys3/analogs_text/year={year}/month={month}/day={day}/analogs_100_filtered.json",
    ...
    build_analogs_text = create_python_in_docker_ssh_command(
        ...
        command="python -m src.cli.run --script=analogs_text --start_date={{ ds }} --end_date={{ ds }} --config=./configs/analogs_text.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=3 * 60 * 60,
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    upload_to_minio_analogs_text = create_ssh_command_operator(
        task_id="upload_to_minio_analogs_text",
        command=(
            f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config='./configs/analogs_text.yaml' --focus_group=2 --coitems"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
