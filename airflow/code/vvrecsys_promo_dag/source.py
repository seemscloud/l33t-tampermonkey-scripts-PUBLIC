...
with DAG(
    ...
    build_promo = create_python_in_docker_ssh_command(
        task_id="build_promo",
        command="python -m vvrecsys.cli.production --config=configs/production/promo.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    deploy_promo = create_ssh_command_operator(
        task_id="deploy_promo",
        command=(
            f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config=configs/production/promo.yaml"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
    ...
    recommends_folder = "/srv/data/production/promo"
    s3_folder = "recsys3/models/promo"
    ...
    copy_files_to_s3 = SSHOperator(
        ...
        aws s3 cp /srv/data/production/promo/ s3://{{ params.bucket_name }}/recsys3/production/{{ macros.ds_add(ds, -1).replace('-', '/') }}/promo/ --recursive --exclude '*' --include '*.h5' --endpoint-url=https://storage.yandexcloud.net
        """,
        params={
