...
with DAG(
    "vvrecsys_emptysearch_new_ranking_dag",
    default_args={
    ...
    repository_name, clone_project = create_clone_repository_operator(
        ...
        dag_id="{{ dag.dag_id }}",
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
        branch_name="feature/new-models-empty-search",
    )
    ...
    # merge_candidates = create_python_in_docker_ssh_command(
    #     task_id="merge_candidates",
    #     command="python -m vvrecsys.cli.merge_candidates --config=configs/merge/tifu_implicit_gtop_fallback.yaml",
    #     docker_container_name=docker_container_name,
    #     cmd_timeout=1 * 60 * 60 * 5,  # 4 hours
    #     ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    # )

    build_emptysearch_new_ranking = create_python_in_docker_ssh_command(
        task_id="build_emptysearch_new_ranking",
        command="python -m vvrecsys.cli.production --config=configs/production/emptysearch_huge_window.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 5,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    build_emptysearch_huge_window_fallback = create_python_in_docker_ssh_command(
        task_id="build_emptysearch_huge_window_fallback",
        command="python -m vvrecsys.cli.production --config=configs/production/emptysearch_huge_window_fallback.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 5,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    # deploy_emptysearch0 = create_ssh_command_operator(
    #     task_id="deploy_emptysearch0",
    #     command=(
    #         f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config=configs/production/emptysearch.yaml --index=0 --focus_group=2"
    #     ),
    #     ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    # )
    ...
    (
        ...
        >> set_date
       # >> merge_candidates
        >> build_emptysearch_huge_window_fallback
        >> build_emptysearch_new_ranking
       # >> build_emptysearch
      #  >> deploy_emptysearch0
        #  >> deploy_emptysearch1
        #  >> deploy_emptysearch2
        #  >> deploy_test_emptysearch0
        #  >> deploy_test_emptysearch1
        #  >> deploy_test_emptysearch2
        >> stop_docker
       # >> copy_files_to_s3
    )
