...
bq_table = 'delivery_limitations_environment'
...
def get_criteo_data_to_local():
    ...
    if data:
        for row in data:
            environments = get_data_from_nested_dict(row, [], 'attributes', 'targeting', 'deliveryLimitations',
                                                     'environments')
            if environments:
...
dag = DAG(
    dag_id='criteo_ads_delivery_limitations_environment_to_bq',
    schedule='15 */6 * * *',
