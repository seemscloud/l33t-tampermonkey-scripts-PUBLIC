...
with DAG(
    "vvrecsys_add_to_cart_dag",
    default_args={
        "depends_on_past": False,
        "on_failure_callback": create_telegram_alert_func(TELEGRAM_TOKEN_BOT),
        "owner": "m.gazoian",
    },
    description="DAG for inference vvrecsys_add_to_cart_dag",
    ...
    build_add_to_cart = create_python_in_docker_ssh_command(
        task_id="build_add_to_cart",
        command="python -m vvrecsys.cli.production --config=configs/production/add_to_cart.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    deploy_add_to_cart0 = create_ssh_command_operator(
        task_id="deploy_add_to_cart0",
        command=(
            f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config=configs/production/add_to_cart.yaml --index=0 --focus_group=1"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    deploy_add_to_cart1 = create_ssh_command_operator(
        task_id="deploy_add_to_cart1",
        command=(
            f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config=configs/production/add_to_cart.yaml --index=1 --focus_group=2"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
    ...
    copy_files_to_s3 = SSHOperator(
        ...
        aws s3 cp /srv/data/production/add_to_cart/ s3://{{ params.bucket_name }}/recsys3/production/{{ macros.ds_add(ds, -1).replace('-', '/') }}/add_to_cart/ --recursive --exclude '*' --include '*.h5' --endpoint-url=https://storage.yandexcloud.net
        """,
        params={
