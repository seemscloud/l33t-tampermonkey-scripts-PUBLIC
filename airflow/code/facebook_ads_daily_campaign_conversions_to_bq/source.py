...
bq_dataset = 'ad_fb'
bq_table = 'daily_campaign_conversions'
    ...
    if raw_data:
        for insight in raw_data:
            ...
            if not conversions:
                continue
            index = 0
            for insight_conversion in conversions:
                df_row_data = {
                    "_fivetran_id": None,
                    "campaign_id": insight.get('campaign_id'),
                    "date": insight.get('date_start'),
                    "index": index,
                    "_1_d_view": insight_conversion.get('1d_view'),
                    "_7_d_click": insight_conversion.get('7d_click'),
                    "_fivetran_synced": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    "action_type": insight_conversion.get('action_type'),
                    "value": insight_conversion.get('value'),
                }
                index += 1
...
dag = DAG(
    dag_id='facebook_ads_daily_campaign_conversions_to_bq',
    schedule='45 0/6 * * *',
