...
bq_dataset = 'ad_fb'
bq_table = 'daily_campaign_actions'
...
def get_list_last_9_days_ago():
    ...
    for d in range(9):
        day = (today - timedelta(days=d+9)).strftime('%Y-%m-%d')
        result.append(day)
    return result
...
def get_facebook_data_to_local():
    ...
    if raw_data:
        for insight in raw_data:
            ...
            index = 0
            for insight_action in actions:
                if insight_action.get('1d_view') is not None and insight_action.get('7d_click') is not None:
                    inline = insight_action.get('value')
                else:
                    inline = None
                df_row_data = {
                    "_fivetran_id": None,
                    ...
                    "index": index,
                    "_1_d_view": insight_action.get('1d_view'),
                    "_7_d_click": insight_action.get('7d_click'),
                    "_fivetran_synced": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    "action_type": insight_action.get('action_type'),
                    "inline": inline,
                    "value": insight_action.get('value'),
                }
                index += 1
...
dag = DAG(
    dag_id='facebook_ads_daily_campaign_actions_to_bq',
    schedule='45 0/6 * * *',
...
if __name__ == "__main__":
    print(get_list_last_9_days_ago())
