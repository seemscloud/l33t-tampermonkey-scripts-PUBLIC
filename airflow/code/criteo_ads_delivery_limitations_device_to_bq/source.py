...
bq_table = 'delivery_limitations_device'
...
def get_criteo_data_to_local():
    ...
    if data:
        for row in data:
            ...
            devices = get_data_from_nested_dict(row, [], 'attributes', 'targeting', 'deliveryLimitations', 'devices')
            if devices:
...
dag = DAG(
    dag_id='criteo_ads_delivery_limitations_device_to_bq',
    schedule='15 */6 * * *',
