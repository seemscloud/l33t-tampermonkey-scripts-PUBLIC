...
def get_criteo_data_to_local():
    ...
    if data:
        for row in data:
            geo_location_countries = get_data_from_nested_dict(row, [], 'attributes', 'geoLocation', 'countries',
                                                               'value', 'values')
            if geo_location_countries:
                for geo_location_country in geo_location_countries:
                    df_row = {
                        "_fivetran_id": None,
...
dag = DAG(
    dag_id='criteo_ads_geolocation_country_to_bq',
    schedule='15 */6 * * *',
