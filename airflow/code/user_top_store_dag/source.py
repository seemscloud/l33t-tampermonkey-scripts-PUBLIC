...
with DAG(
    ...
    build_user_store_map = create_python_in_docker_ssh_command(
        task_id="build_user_store_map",
        command="python -m src.cli.user_store_map_run --config='./configs/user_store_map.yaml' --base_path='/app/data/raw/sales' --date={{ (params.logic_date if params.logic_date else macros.ds_add(ds, -2)) | replace('-', '/') }}",
    ...
    # Dependencies
    (start >> clone_project >> create_env_file >> start_docker >> build_user_store_map >> stop_docker >> end)
