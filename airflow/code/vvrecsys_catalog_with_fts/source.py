...
with DAG(
    ...
    build_fts = create_python_in_docker_ssh_command(
        task_id="build_fts",
        command="python -m src.cli.run_fts --script=fts --end_date={{ macros.ds_add(ds, -1) }}",
    ...
    (start >> check_file_sensor >> clone_project >> create_env_file >> start_docker >> build_fts >> stop_docker)
