...
with DAG(
    "vvrecsys_emptysearch_new_filter_dag",
    default_args={
    ...
    repository_name, clone_project = create_clone_repository_operator(
        ...
        dag_id="{{ dag.dag_id }}",
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
        branch_name='feature/new-filtering',
    )
    ...
    build_emptysearch_new_filtering = create_python_in_docker_ssh_command(
        ...
        command="python -m vvrecsys.cli.production --config=configs/production/empty_search_new_filtering.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 5,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
