...
def get_criteo_data_to_local():
    ...
    url = "https://api.criteo.com/2024-07/marketing-solutions/campaigns/search"
    header = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        'Authorization': f'Bearer {access_token}',
    ...
    if response_data['data']:
        for row in response_data['data']:
            ...
            attr_spend_limit = attribute_data.get('spendLimit')
            df_row_data = {
                "id": row.get("id"),
                ...
                "objective": attribute_data.get('goal'),
                "spend_limit_amount": attr_spend_limit.get('spendLimitAmount').get('value') if attr_spend_limit.get(
                    'spendLimitAmount').get('value') is not None else 0.0,
                "spend_limit_renewal": attr_spend_limit.get('spendLimitRenewal'),
                "spend_limit_type": attr_spend_limit.get('spendLimitType'),
            }
...
dag = DAG(
    dag_id='criteo_ads_campaign_to_bq',
    schedule='15 */6 * * *',
...
gcs_data_to_bq = GCSToBigQueryOperator(
    ...
    schema_fields=[
        ...
        {
            "name": "spend_limit_amount",
            "mode": "NULLABLE",
            "type": "FLOAT",
        },
        {
            "name": "spend_limit_renewal",
            "mode": "NULLABLE",
            "type": "STRING",
        },
        {
            "name": "spend_limit_type",
            "mode": "NULLABLE",
            "type": "STRING",
        }
    ],
