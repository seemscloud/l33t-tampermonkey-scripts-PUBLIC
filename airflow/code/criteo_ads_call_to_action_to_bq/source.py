...
def get_criteo_data_to_local():
    ...
    for advertiser_id in advertiser_ids:
        url = f"https://api.criteo.com/2024-07/marketing-solutions/advertisers/{advertiser_id}/creatives"
        response = requests.get(
            url=url,
            headers=header,
        )
    ...
    if data:
        for row in data:
            if (row.get('attributes', {}).get('dynamicAttributes', {}).get('callsToAction', [])
                    or row.get('attributes', {}).get('adaptiveAttributes', {}).get('callsToAction', [])):
                df_row_data = {
                    "_fivetran_id": None,
                    "_fivetran_deleted": False,
                    "_fivetran_synced": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    "creative_id": row.get('id', None),
                }
...
dag = DAG(
    dag_id='criteo_ads_call_to_action_to_bq',
    schedule='15 */6 * * *',
