...
bq_dataset = 'ad_fb'
bq_table = 'daily_campaign_purchase_roas'
...
def get_facebook_data_to_local():
    ...
    if raw_data:
        for insight in raw_data:
            values = insight.get('purchase_roas', [])
            if not values:
                continue
            index = 0
            for insight_purchase_roas in values:
                df_row_data = {
                    "_fivetran_id": None,
                    "campaign_id": insight.get('campaign_id'),
                    "date": insight.get('date_start'),
                    "index": index,
                    "_1_d_view": insight_purchase_roas.get('1d_view'),
                    "_7_d_click": insight_purchase_roas.get('7d_click'),
                    "_fivetran_synced": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    "action_type": insight_purchase_roas.get('action_type'),
                    "value": insight_purchase_roas.get('value'),
                }
                index += 1
...
dag = DAG(
    dag_id='facebook_ads_daily_campaign_purchase_roas_to_bq',
    schedule='45 0/6 * * *',
