...
from operators.remote_operator import RemoteFileSensor, create_clone_repository_operator, create_ssh_command_operator
...
with DAG(
    "dev_recsys_new_ranking_features_dag",
    default_args={
    ...
    create_env_file = create_ssh_command_operator(
        ...
        command=(
            ...
            f"AWS_SECRET_ACCESS_KEY={AWS_SECRET_ACCESS_KEY}' > recsys_ranking_features/.env"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
    ...
    build_docker_image = create_ssh_command_operator(
        task_id="build_image",
        command=f"docker build --no-cache --rm -t {IMAGE_NAME} recsys_ranking_features/.",
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
    ...
    create_query_features = create_ssh_command_operator(
        ...
        command=f"docker exec -i {docker_container_name} python -m cli  --config=src/config/config_new.yaml --date {{{{ ds }}}} --group=query",
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    create_queryitem_features = create_ssh_command_operator(
        task_id="create_queryitem_features",
        command=f"docker exec -i {docker_container_name} python -m cli  --config=src/config/config_new.yaml --date {{{{ ds }}}} --group=queryitem",
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    create_querycat_features = create_ssh_command_operator(
        task_id="create_querycat_features",
        command=f"docker exec -i {docker_container_name} python -m cli  --config=src/config/config_new.yaml --date {{{{ ds }}}} --group=querycat",
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )
