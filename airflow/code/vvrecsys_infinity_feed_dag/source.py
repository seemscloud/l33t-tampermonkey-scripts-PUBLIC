...
with DAG(
    ...
    build_empty_search_card = create_python_in_docker_ssh_command(
        task_id="build_empty_search_card",
        command="python -m vvrecsys.cli.production --config=configs/production/empty_search_card.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    build_empty_search_cart = create_python_in_docker_ssh_command(
        task_id="build_empty_search_cart",
        command="python -m vvrecsys.cli.production --config=configs/production/empty_search_cart.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    build_empty_search_catalog = create_python_in_docker_ssh_command(
        task_id="build_empty_search_catalog",
        command="python -m vvrecsys.cli.production --config=configs/production/empty_search_catalog.yaml",
        docker_container_name=docker_container_name,
        cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
    )

    deploy_empty_search_card = create_ssh_command_operator(
        task_id="deploy_empty_search_card",
        command=(
            f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config=configs/production/empty_search_card.yaml --index=0 --focus_group=1"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
        cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
    )

    deploy_empty_search_cart = create_ssh_command_operator(
        task_id="deploy_empty_search_cart",
        command=(
            f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config=configs/production/empty_search_cart.yaml --index=0 --focus_group=1"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
        cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
    )

    deploy_empty_search_catalog = create_ssh_command_operator(
        task_id="deploy_empty_search_catalog",
        command=(
            f"docker exec -i {docker_container_name} python -m vvrecsys.cli.minio --config=configs/production/empty_search_catalog.yaml --index=0 --focus_group=1"
        ),
        ssh_conn_id=REMOTE_HOST_MACHINE_SSH_CONNECTION,
        cmd_timeout=1 * 60 * 60 * 4,  # 4 hours
    )
    ...
    recommends_folder = "/srv/data/production/infinity_feed"
    s3_folder = "recsys3/models/infinity_feed"
    ...
    copy_files_to_s3 = SSHOperator(
        ...
        aws s3 cp /srv/data/production/infinity_feed/ s3://{{ params.bucket_name }}/recsys3/production/{{ macros.ds_add(ds, -1).replace('-', '/') }}/infinity_feed/ --recursive --exclude '*' --include '*.h5' --endpoint-url=https://storage.yandexcloud.net
        """,
        params={
    ...
    (
        ...
        >> start_docker
        >> set_date
        >> build_empty_search_card
        >> build_empty_search_cart
        >> build_empty_search_catalog
        >> deploy_empty_search_card
        >> deploy_empty_search_cart
        >> deploy_empty_search_catalog
        >> stop_docker
        >> copy_files_to_s3
    )
