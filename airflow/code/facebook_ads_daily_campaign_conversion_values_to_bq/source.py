...
bq_dataset = 'ad_fb'
bq_table = 'daily_campaign_conversion_values'
    ...
    if raw_data:
        for insight in raw_data:
            ...
            if not conversion_values:
                continue
            index = 0
            for insight_conversion_value in conversion_values:
                df_row_data = {
                    "_fivetran_id": None,
                    "campaign_id": insight.get('campaign_id'),
                    "date": insight.get('date_start'),
                    "index": index,
                    "_1_d_view": insight_conversion_value.get('1d_view'),
                    "_fivetran_synced": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    "action_type": insight_conversion_value.get('action_type'),
                    "value": insight_conversion_value.get('value'),
                    "_7_d_click": insight_conversion_value.get('7d_click'),
                }
                index += 1
...
dag = DAG(
    dag_id='facebook_ads_daily_campaign_conversion_values_to_bq',
    schedule='45 0/6 * * *',
